...
def test_trades_past_accepted_future_rejected(sm):
    """Журналы (trades/events) — история: старый честный ts легален (journal-sync бэкфиллит
    биржу в свежую БД после редеплоя; урок Борса 2026-07-21), будущее за окном — 422."""
    iid, tok = _mk_instance_token(sm)
    c = TestClient(create_app())
    old = [{"ts": _iso(-3 * 24 * 3600), "exec_id": "old1", "symbol": "BTCUSDT",
