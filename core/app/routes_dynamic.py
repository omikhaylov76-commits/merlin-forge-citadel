...
СМОТРИТ: капитализация/оборот/возраст) — канал 0018, не смешиваем (ADR-0018 п.3 зеркально).
Только операторский токен (закон №5: портал — лишь PAUSE/STOP_CLOSE). Изменение — audit (№4).
...
DYNAMIC_DEFAULTS: dict = {
    "min_score": 0,     # DYNAMIC_MIN_SCORE — доп-порог ПОВЕРХ дозорного (0 = без фильтра)
...
class DynamicSettings(BaseModel):
    ...
    stack_max: int = Field(ge=1, le=100)      # предохранитель 1..100 (100000 НЕ пройдёт)
...
def get_dynamic_settings_self(
    ...
    скоупа: отдаём ТОЛЬКО движко-критерии — дозор/риск каналом НЕ ходят (их путь — свои каналы)."""
