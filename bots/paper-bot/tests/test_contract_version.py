...
Не даёт разойтись версии бота и версии контракта: бамп v0→v1 роняет тест, если забыли константу
(или наоборот). Pydantic-зеркало снимков — на стороне ядра (#52), картридж намеренно без pydantic —
здесь только schema↔константа.
"""
...
_SCHEMAS = (
    ...
    "command.schema.json",
    "telemetry-scout.schema.json",  # прибивает v1 целиком (paper-bot скаут не производит)
)
