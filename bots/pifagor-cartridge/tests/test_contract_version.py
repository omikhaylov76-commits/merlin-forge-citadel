...
1) CONTRACT_VERSION == версия в `$id` ВСЕХ схем (включая пятый, scout) — бамп v0→v1 роняет тест,
   если забыли константу/схему.
2) Scout-схема валидна как JSON Schema 2020-12 и её examples проходят (schema-first гвоздь).
Полное Pydantic-зеркало scout-снимка — на стороне ядра-приёмника (#52), где есть pydantic; картридж
снимок ПРОИЗВОДИТ и валидирует jsonschema (минимум зависимостей) — здесь schema↔структура.
"""
