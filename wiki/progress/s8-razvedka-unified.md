...
- **Живой boot Борса ЧИСТ:** `движок [LIVE demo] рестартов=0`, `reconcile_on_start: {}`, новая строка
  `scout: снимков 2, правда движка за 0.00с` (замер дёшев), 0×422, 0 трейсбеков.
- **🔴→✅ Живой баг (RED-фикс, `44a9df2`): доска Разведки висла после смены порогов дозора.** Оператор
  видел 6 монет в карточке бота (engine_state, свежий каждый тик), но Разведка пуста. Причина —
  триггер `_push_scout` слал снимки ТОЛЬКО на новый `scan_ms`/held/стек; смена порогов пересобирает
  находки скаута в ТОЙ ЖЕ 4h-границе (scan_ms не двигается) → пуш не уходил (в логах ЕДИНСТВЕННЫЙ
  scout-POST 16:46, дальше тишина при 39 снимках/цикл). Фикс: `_scout_sig` — пуш на смену СОДЕРЖИМОГО
  (символ/стадия/вердикт/ордера, без timestamp-дрожи). Тесты +2. Деплой Борса свежим образом.
- Осталось: визуальная сверка Оператором (глазами Борса на консоли — гейт живого показа).
