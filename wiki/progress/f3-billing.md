...
## Статус (2026-07-14)
Данные (0006: contracts/billing_periods/cashflows + immutable-триггер + EXCLUDE overlap) + движок HWM
(billing.py: compute_period + close_period со снапшотом fee_pct/аудитом) — готовы, CI зелёный (Postgres).
Эталонные тесты #27 сходятся. **Обязательное адверс-ревью (агент) пройдено ДВАЖДЫ:** 1-й проход нашёл
🔴 critical (закрытие не по порядку → неверная immutable-комиссия) + 3×🟡 — все закрыты (порядок+FOR UPDATE,
v1-гейт profit_hwm/hurdle0/mgmt0, EXCLUDE-constraint, ниты). 2-й проход подтвердил: закрыто, регрессий нет,
«готово к merge». Остаточные не-блокеры (fail-safe): политика разрыва при паузе аккаунта (→ Куратору),
forward-контракт генератора периодов, дубль-гейт на подписании договора.
