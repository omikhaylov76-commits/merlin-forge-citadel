...
- [~] 6. code-review → ⛔ merge в main → деплой Борса (`SIGNAL_JOURNAL_ENABLED=1`+`SIGNAL_JOURNAL_CORE=BORS`)
      ...
      core 35, ruff/tsc чисты. **MERGED в main @ `48620a7`** (--no-ff, слово Оператора) →
      **ЗАДЕПЛОЕНО ЦЕЛИКОМ** (ядро миграция 0016 · Борс `SIGNAL_JOURNAL_ENABLED=1` первым · консоль):
      журнал `boot ok`, пуш 202, торговля не тронута (LIVE/cap/reconcile), стражи неизменны, 119 событий и растёт.
      Осталось: сверка полноты N дней (приёмка Куратора) → порция №4.
