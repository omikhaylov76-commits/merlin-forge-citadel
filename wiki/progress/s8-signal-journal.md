...
- [~] 6. code-review → ⛔ merge в main → деплой Борса (`SIGNAL_JOURNAL_ENABLED=1`+`SIGNAL_JOURNAL_CORE=BORS`)
      ...
      core 35, ruff/tsc чисты. **MERGED в main @ `48620a7`** (--no-ff, слово Оператора). Осталось:
      деплой спокойным окном (ядро+миграция 0016 → Борс первым → консоль) → сверка полноты N дней (приёмка Куратора).
