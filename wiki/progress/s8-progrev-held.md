...
## SHA
- v1 (правка tick) — НЕ работал (находка разведки: движок читает coins.json на старте и стартует
  раньше адаптера + gen=0 не рестартит + defer при позициях). Заменён на v2 (boot-шаг).
- v2: ветка `task/s8-progrev-held` `2c03a31` → merge `66d23ca` (push `87c99ad..66d23ca`). Картридж 239
  passed, ruff чист, страж-дрейфа=3. Независимый code-review 0 блокеров (+2 low: best-effort main, фильтр held).
- Образ `:main` собран CI. Деплой Борса `cd8d0534` (env те же +cap=24), стражи `2730f90c`/`e8ffd609`
  ДО==ПОСЛЕ. **Живая сверка 11:39:12 — прогрев сработал:** `dynamic: прогрев held ДО движка (5 монет):
  1INCH,BANK,KAITO,VVV,ZAMA` → `knobs(eff) … монет=5` → `стек исполнения meta 5/5` (НЕ дефолт-16) →
  reconcile 5/5 kept. **Окно неведения позиций закрыто (движок ведёт held с 1-й минуты).**
- ⚠️ Отклонение от плана v1 → флаг Куратору в QUEUE (его домен: подпись подхода). Полный персист
  scout.db (окно для бота БЕЗ позиций) — отдельный техдолг, не здесь.
