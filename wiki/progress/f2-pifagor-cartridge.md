...
- [x] 5. Образ-обёртка: Dockerfile (context=bots/: vendor→/pifagor + адаптер) + start.sh (движок dry-run demo
       + адаптер) + .dockerignore. Safe-режим. **Собран локально (479MB), импорт в образе проверен** (71a1e58 +
       append-фикс коллизии пакета `app`).
- [x] 6a. **ЖИВОЙ СКВОЗНЯК против РЕАЛЬНОГО ядра** (локально, session 5): картридж-процесс против uvicorn-ядра
       на dev-БД + засеянная SQLite Пифагора. ✅ heartbeat 204 · equity 202 · trades/events приняты (health=ok) ·
       pause→PAUSE_ENABLED=True (delivered→acked, аудит) · stop_close→killswitch-латч=1 (acked, картридж встал).
       Весь контур S4 доказан на живом ядре (не мок). Скрипты — scratchpad (вне репо).
- [ ] 6b. ⛔ **Деплой Railway (облако) — ГЕЙТ ОПЕРАТОРА:** конвейер рабочий (RailwayDriver подтверждён на
       paper-bot, образ собран), НО нужны (1) согласие на облачный compute + (2) demo-ключи Bybit (не боевые)
       для БУТА воркера в образе — только Оператор кладёт в env (закон №2). Без ключей — адаптер-only (деградир.).
       Вынесено в QUEUE. Реальные ключи/торговля = ОТДЕЛЬНЫЙ гейт go-live.
- [ ] 7. (отложено, гейт go-live) Конверт-шифрование ключей биржи end-to-end (ADR-0004/0010) + тесты.

## Итог session 5
Адаптер картриджа Пифагора функционален и доказан: 67 тестов (юнит + parity vs реальный build_monitor +
schema-conformance) + ЖИВОЙ сквозняк телеметрии/команд против настоящего ядра. Образ собирается. Осталось
одно — облачный деплой на Railway (гейт Оператора: compute + demo-ключи). Ветка готова к слиянию в main.
