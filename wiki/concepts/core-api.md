---
type: concept
title: API ядра (core) — состояние MFC-002
tags: [core, api, auth, scheduler]
updated: 2026-07-11
sources: [core/, decisions/0008, 0009, 0010, 0012]
---
# API ядра (core)

Реализовано в MFC-001 (скелет) + MFC-002 («часовой»). FastAPI app-factory (`create_app`),
структурный JSON-лог с request-id. Это состояние на сегодня; полный домен и швы —
[domain-model](domain-model.md) / [seams](seams.md).
...
## Часовой (core-scheduler, MFC-002)
...
Реестр свёрток — плагин-точка; первая свёртка (stale-скан heartbeat) — следующая фича (нужна
схема инстансов). Падение свёртки изолировано: сосед не роняет цикл.
...
Свёртка stale-скан heartbeat (нужна схема инстансов) + outbox-алерты; internal API jobs для
оркестратора; домен instances/jobs/telemetry/billing (свои миграции); биржевые ключи (Ф2);
биллинг HWM (Ф3).
