...
- [HANDOFF_2026-07-23_session_18](handoffs/HANDOFF_2026-07-23_session_18.md) — **ADR-0023 cap→24 ОТГРУЖЕН + порция №2 «прогрев held» ОТГРУЖЕНА (план исправлен разведкой: boot-шаг вместо tick).** Обе живы на облаке · cap 16→24 = 3-я санкц-дельта генома (merge `00942f1`, сверка `knobs(eff) cap=24`) · прогрев held ДО движка через `PifagorReader(owner=False)` (merge `66d23ca`, сверка: движок стек 5/5 held, не дефолт-16, окно неведения закрыто) · диагноз «нет сетапов»=таймзона+окно скана · 3 пункта Куратору (порция №2 v2 подпись · console-слайдер `knobs.ts:16` · cap-остаток телеметрия). main @ 8d41d5f.
- [HANDOFF_2026-07-23_session_19](handoffs/HANDOFF_2026-07-23_session_19.md) — **🏁 S8 порция №3 «Сигнальный журнал» ВЕСЬ КОД готов на ветке `task/s8-signal-journal` (НЕ влит) + аксиома пере-якоря V3 + Борс/Персиваль→V3.** Диагностика WLFI (warm.classify на реальных 4h Bybit — сетап верен; корень = env-флаг только при редеплое) · аксиома пере-якоря (`entities/pifagor-engine.md`+память) · Борс+Персиваль на V3 (демо, чистый боот) · порция №3: схема+ядро(миграция 0016, натур. ключ+guard эпохи)+адаптер(`SignalJournalDeriver`, П1-П4)+readout+консоль «Сигналы»; тесты картридж 252/core 34/журнал 13 vs вендор, ruff/tsc чисты. Дальше: code-review→merge→деплой Борса (`SIGNAL_JOURNAL_ENABLED=1`). main @ 19d2bcc, ветка @ 0d99c53.
...
- [0025](decisions/0025-signal-journal-channel.md) — Сигнальный журнал: 6-й канал Контракта (Этап 1 1-to-N); append-only, дедуп натур. ключом (instance,src_table,src_id), seq=порядок, guard эпохи fail-closed, гейт деф.OFF, 0-vendor; ведение → вариант C (ADR-0024).
...
- [progress/s8-signal-journal](progress/s8-signal-journal.md) — порция №3 Сигнальный журнал (Этап 1 переката 1-to-N): разведка (движок дискретно журналит signals/fills/events/closed_trades; гап ведения+полноты) + схема событий НА ПОДПИСЬ Куратора; кода нет.
