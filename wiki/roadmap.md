...
- [x] S7 карточка бота (engine_state: картридж пуш + ядро 0011 + BotCard консоли) — merged e1e3ce8, обкатана на Галахаде вживую; Персиваль по совместной отмашке — 2026-07-17
...
- 🔩 Живая цена + свежесть карточки бота (запрос Оператора 2026-07-23, «раздражает, но терпимо»): текущая цена позиций из ПУБЛИЧНОГО Bybit в консоли (1–2с REST/WS, без ключей, минуя бота) + живой P&L; ужать каденцию engine_state (MF_TICK_S 5→~3с + консоль-фетч ~5→3с) для свежести позиций (пол = опрос Bybit внутри Пифагора, Закон 6); CORS Bybit→консоль проверить (иначе прокси ядра). Отдельной порцией.
