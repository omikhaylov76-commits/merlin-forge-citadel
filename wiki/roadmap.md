...
- [~] #50 ADR-0016 + scout-канал Контракта v1 (схема `telemetry-scout` + бамп v0→v1 + sync-гвозди; 0 строк в vendor/) — на приёмке Куратора
- [ ] #51 Изоляция картриджа: fail-closed `SCOUT_ENABLED`-гейт + отдельная `scout.db` (DB_PATH) + супервизор скаута в start.sh — todo
- [ ] #52 Ядро + readout: приёмная ручка/таблица scout-снимка + fleet-эндпоинт + Pydantic-зеркало (enforcement версии) — todo
- [ ] #53 Консоль-график сетапа: живой экран Разведки (свечи + A→B + Fib-ноги + стоп) вместо фикстур — todo
