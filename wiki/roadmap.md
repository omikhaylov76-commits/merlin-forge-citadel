...
## Ф4 — Сборка консоли + портал (in-progress, старт #32 2026-07-15)
...
- [x] Кузница · Конструктор профиля (main b1a31ce): 23 крутилки движка (реестр lib/knobs, эталон @b75bd17),
      полки осн./эксп., ⚠-опасные, счётчик отличий, экспертный режим, OOS-паспорт. Скринер вселенной/матрица
      входа «с какой ноги» — это ЧАСТЬ 2 (доработка движка, форк эталона), не v1-крутилки — позже.
- [ ] Кузница · Разведка (kanban скаута) + Профили (библиотека рецептов) — витринные экраны, по макету
- [ ] Экраны: Отчёты / Тревоги (readout застрявшего биллинга сюда) / Настройки
- [ ] Портал клиента: read-only доходность/статус + PAUSE (мгновенно) + STOP_CLOSE (двойное подтв.) через API+аудит
- Помощники Кавалл + Архимед — отдельной фазой ПОСЛЕ консоли (#32)
